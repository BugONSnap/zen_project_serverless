...
<div class="min-h-screen text-white font-medium relative dashboard-bg" style="background: linear-gradient(135deg, #0f172a 0%, #1a1f2e 50%, #111827 100%); overflow: visible; font-family: poppins;">
    ...
    <DashboardHeader title={data.category.name} user={data.user} pageName="Advanced JavaScript" />
    <main class="max-w-6xl mx-auto px-4 py-10 space-y-10">
        ...
        <section class="rounded-3xl border border-white/15 bg-black/30 p-6 shadow-xl backdrop-blur space-y-6" id="identification-area">
            ...
            {#if identificationPages > 1}
                <div class="flex items-center justify-center gap-4 rounded-2xl border border-white/15 bg-[#090303]/70 p-4 text-sm text-white/70 shadow-lg backdrop-blur">
                    <button
