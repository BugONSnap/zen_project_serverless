<script lang="ts">
    ...
    import UserManualModal from '$lib/components/UserManualModal.svelte';
    import { goto } from '$app/navigation';
    import { onMount } from 'svelte';

    export let data: PageData;

    const MANUAL_SESSION_KEY = 'zentry-manual-seen';
    ...
    const isAdmin = (data.user.adminLevel ?? 2) <= 1;
    ...
    function onManualClose() {
        showManual = false;
        try {
            sessionStorage.setItem(MANUAL_SESSION_KEY, '1');
        } catch {
            /* ignore */
        }
    }
    ...
    onMount(async () => {
        try {
            if (!sessionStorage.getItem(MANUAL_SESSION_KEY)) {
                showManual = true;
            }
        } catch {
            showManual = true;
...
<UserManualModal
    bind:open={showManual}
    username={data.user.username}
    {isAdmin}
    on:close={onManualClose}
/>
...
<div class="min-h-screen text-white font-medium relative overflow-hidden dashboard-bg" style="background: linear-gradient(135deg, #0f172a 0%, #1a1f2e 50%, #111827 100%); font-family: poppins;">
    ...
    <main class="max-w-6xl mx-auto px-4 py-10 space-y-10 relative z-10">
        <div class="flex justify-end">
            <button
                type="button"
                class="guide-btn rounded-full px-4 py-2 text-sm font-semibold flex items-center gap-2"
                on:click={openManual}
            >
