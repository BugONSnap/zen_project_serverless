...
<div class="min-h-screen text-white font-medium relative dashboard-bg" style="background: linear-gradient(135deg, #0f172a 0%, #1a1f2e 50%, #111827 100%); overflow: visible; font-family: poppins;">
    ...
    <DashboardHeader title="HTML Quizzes" user={data.user} pageName="HTML" />
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                {:else}
                    {#each easyPaginated as quiz}
                        <button
                            class={`relative rounded-2xl border border-white/15 bg-black/40 p-4 text-left shadow-lg backdrop-blur transition hover:border-emerald-400/60 ${isAnswered(quiz.id) ? 'opacity-40 pointer-events-none' : ''}`}
